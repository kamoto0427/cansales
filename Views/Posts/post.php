...
$err = [];
  ...
  if($_SERVER['REQUEST_METHOD'] === 'POST'){
      ...
      if(count($err) === 0) {
        ...
        $result = $obj->createList($company_id,$_POST,$save_path, $j_tags);
        $_SESSION['list_msg'] = "投稿が完了しました！";
  ...
  <div class="p-wrapper">
    <div class="p-container">
      ...
      <form action="" method="POST" enctype="multipart/form-data" id="p-form">
        ...
        <div class="tag-lebel">
          <label for="kome"><font color="red">*</font></label>
          <label for="tag" class="form-label">タグの候補を選択してください。(最大5つまで選択可能)※最低1つは選択必須</label>
          <?php if(isset($err['tags'])): ?>
            <p class="err"><font color="red"><?php echo $err['tags'] ?></font></p>
          <?php endif; ?>
        </div>
        <div class="mb-3 tag-form">
          <input name="tags" class='tag_field' placeholder="ここをクリック" value='<?php if(isset($tags)) echo h($tags) ?>'>
        </div>
        <!-- サービス詳細 -->
        <div class="mb-3">
          <label for="kome"><font color="red">*</font></label>
          <label for="contents" class="form-label">詳細</label>
          <p id="contents_btn">基本テンプレートを使用する</p>
          <?php if(isset($err['text'])): ?>
            <p class="err"><font color="red"><?php echo $err['text'] ?></font></p>
          <?php endif; ?>
          <textarea class="form-control" id="contents" name="text" rows="20" placeholder="サービスの内容、利用者のメリット、アピールしたい実績などを3000文字以内で具体的に書いてください。具体的でわかりやすいほど、リストを見た企業がリクエストをしてくれる確率がアップします。"><?php if(isset($text)) echo h($text) ?></textarea>
        </div>
        <!-- カテゴリー -->
        ...
        <select class="form-select" id="category-select" name="category">
          <?php if(isset($_POST['category'])): ?>
            <option selected value="<?php echo h($_POST['category']); ?>"><?php echo h($obj->setCategoryName($category)); ?></option>
          <?php else: ?>
        ...
        <?php if(isset($err['price'])): ?>
          ...
          <?php endif; ?>
        <div class="mb-3">
          <input type="text" class="form-control" id="p-price_form" name="price" placeholder="29800" value="<?php if(isset($price)) echo h($price) ?>">
        </div>
        <!-- 画像 -->
        ...
        <div class="p-btn">
          ...
          <button type="submit" class="btn btn-primary" id="p-btn_btn" name="b">リストを投稿する</button>
        </div>
      </form>
    </div>
  </div>

...
// ログインチェック
$check_login = $obj_c->checkLogin();
if(!$check_login) {
  $_SESSION['login_err'] = "ユーザーを登録してログインしてください。";
  header('Location: ../../Companies/register/login_form.php');
  exit();
}
...
<body>
<p id="list_success"><b><?php if(isset($_SESSION['list_msg'])) echo h($_SESSION['list_msg']) ?></b><?php unset($_SESSION['list_msg']) ?></P>
<?php include(ROOT_PATH .'Views/common/header.php'); ?>
<?php if(($company['role']) == 0): ?>
  <p id="login_name"><b><?php echo h($_SESSION['login_company']['company_name']) ?>さん、こんにちは！</b>&emsp;&emsp;&emsp;管理者でログイン中</P>
<?php endif; ?>
<?php if(($company['role']) == 1): ?>
  <p id="login_name"><b><?php echo h($_SESSION['login_company']['company_name']) ?>さん、こんにちは！</b>&emsp;&emsp;&emsp;営業(サービス提供会社)でログイン中</P>
<?php endif; ?>
<?php if(($company['role']) == 2): ?>
  <p id="login_name"><b><?php echo h($_SESSION['login_company']['company_name']) ?>さん、こんにちは！</b>&emsp;&emsp;&emsp;リクエスト(サービスを探す会社)でログイン中</P>
<?php endif; ?>
          ...
          <?php foreach($result as $val): ?>
            <div class="lists">
              <div class="l-left-contents">
                  <a href="../Companies/mypage/show.php?id=<?php echo h($val['company_id']) ?>" id="list-company"><?php echo h($val['company_name']) ?></a>
              ...
              <div class="l-right-contents">
                <div class="l-right-contents_top">
                  ...
                  <form class="favorite_count" action="" method="POST">
                    <button type="button" name="favorite" class="favorite_btn" data-post_id="<?php echo h($val['id']) ?>" data-c_id="<?php echo h($_SESSION['login_company']['id']) ?>">
                      <?php if ($obj_f->check_favolite_duplicate($_SESSION['login_company']['id'], $val['id'])): ?>
